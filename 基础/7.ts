...
namespace b { //命名空间， 当一个文件里起了两个同名的类，可以用命名空间分割
...
// 属性装饰器（装饰属性，装饰方法）
// 属性装饰器在运行时可能会被当作函数被调用，它可能传入以下参数
namespace d {
  /*  
   * target 如果装饰的是实例属性的话，target是构造函数的原型
   * propertyKey
  */
  ...
  function noEnumerable(target: any, propertyKey: string, descriptor: PropertyDescriptor) { // PropertyDescriptor 属性描述器
    console.log(target, propertyKey)
...
// 参数装饰器
// 用处，在IOC容器里大放异彩，Nest.js 大量用到了参数装饰器
namespace e {
